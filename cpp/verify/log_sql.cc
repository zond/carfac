...
std::unique_ptr<sqlite::execute> insert_value;
int log_sql_step = 0;
int log_sql_ear = 0;
bool log_sql_enabled = true;
...
void log_init() {
	...
	sqlite::execute(*conn.get(), "create table log_values (step int, ear int, name varchar, value float)", true);
	sqlite::execute(*conn.get(), "create unique index log_values_index on log_values (step, ear, name)", true);
	insert_value.reset(new sqlite::execute(*conn.get(), "insert into log_values (step, ear, name, value) values (?, ?, ?, ?)"));
}
